---
name: admin-design-visualization
...
- 最短发起格式可写成 `$admin-design-visualization 判断是否上图` 或 `$admin-design-visualization decide visualization`
...
图表建议要讲“用户看完这块信息后要做什么动作”。不要只回答“上不上图”，也不要把图表类型当成目标本身。
