...
- 最短发起格式可写成 `$admin-design-motion 补必要反馈` 或 `$admin-design-motion refine motion feedback`
...
动效建议要说明“没有它用户会不会漏掉状态变化”。不要只说保留或删除，也不要把装饰性动效包装成体验升级。
