...
- 需要保留项目经验、评审结论或运行轨迹时，按 `references/memory-and-review-ledger.md` 写入本地账本；未审核记录不能自动提升到公开规则层。
...
- 本地记忆默认写入 `${CODEX_HOME:-$HOME/.codex}/admin-design/projects/<project-slug>/`，不提交到公开仓库。
